...
const raisedToTheThird = (arr) => {
  let poweredArray = [];
  arr.forEach((item) => {
    poweredArray.push(Math.pow(item, 3));
  });
  return poweredArray;
};
...
const addOne = (arr) => {
    let incrementArray = arr.map((item) => item + 1);
...
const addQuestion = (arr) => {
  let questionArr = arr.map((item) => `${item}?`);
  return questionArr;
